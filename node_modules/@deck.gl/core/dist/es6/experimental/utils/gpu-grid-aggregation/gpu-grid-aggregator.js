...
import { getFloatTexture, getFramebuffer, getFloatArray, updateBuffer } from './gpu-grid-aggregator-utils.js';
export default class GPUGridAggregator {
  ...
  constructor(gl, opts = {}) {
    ...
    this._hasGPUSupport = isWebGL2(gl) && // gl_InstanceID usage in min/max calculation shaders
    hasFeatures(this.gl, FEATURES.BLEND_EQUATION_MINMAX, // set min/max blend modes
  ...
  validateProps(aggregationParams, opts) {
    const changeFlags = aggregationParams.changeFlags,
          projectPoints = aggregationParams.projectPoints,
          gridTransformMatrix = aggregationParams.gridTransformMatrix;
    assert(changeFlags.dataChanged || changeFlags.viewportChanged || changeFlags.cellSizeChanged); // assert for required options

    assert(!changeFlags.dataChanged || opts.positions && opts.weights && (!opts.projectPositions || opts.viewport) && opts.cellSize);
    assert(!changeFlags.cellSizeChanged || opts.cellSize); // viewport need only when performing screen space aggregation (projectPoints is true)
  ...
  calculateAggregationData(opts) {
    ...
    for (const id in weights) {
      ...
      assert(size >= 1 && size <= 3); // Fill RGB with weights
  ...
  initCPUResults(opts) {
    ...
    const results = {}; // setup results object
    ...
    for (const id in weights) {
      ...
      if (combineMaxMin) {
        maxMinData = getFloatArray(maxMinData, ELEMENTCOUNT); // RGB for max value

        maxMinData.fill(-Infinity, 0, ELEMENTCOUNT - 1); // Alpha for min value
  ...
  runAggregationOnCPU(opts) {
    ...
    const results = this.initCPUResults(opts); // screen space or world space projection required
  ...
  renderToWeightsTexture(opts) {
    ...
    if (operation === AGGREGATION_OPERATION.MEAN) {
      ...
      meanTransform.run(); // update framebuffer with mean results so readPixelsToBuffer returns mean values

      framebuffers[id].attach({
        [GL.COLOR_ATTACHMENT0]: textures[id]
      });
    }
  }
  ...
  setupFramebuffers(opts) {
    ...
    for (const id in weights) {
      ...
      framebuffers[id] = framebuffers[id] || getFramebuffer(this.gl, {
        id: `${id}-fb`,
        width: numCol,
        height: numRow,
        texture
      ...
      equations[id] = EQUATION_MAP[operation]; // For min/max framebuffers will use default size 1X1

      if (needMin || needMax) {
        if (needMin && needMax && combineMaxMin) {
          maxMinFramebuffers[id] = maxMinFramebuffers[id] || getFramebuffer(this.gl, {
            id: `${id}-maxMinFb`
          });
        } else {
          if (needMin) {
            minFramebuffers[id] = minFramebuffers[id] || getFramebuffer(this.gl, {
              id: `${id}-minFb`
            });
          }

          if (needMax) {
            maxFramebuffers[id] = maxFramebuffers[id] || getFramebuffer(this.gl, {
              id: `${id}-maxFb`
            });
          }
        }
  ...
  setupWeightAttributes(opts) {
    ...
    for (const id in weights) {
      const values = weights[id].values; // values can be Array, Float32Array or Buffer
  ...
  updateModels(opts) {
    ...
    if (changeFlags.dataChanged || !positionsBuffer) {
      ...
      const vertexCount = positions.length / 2; // positionsBuffer = new Buffer(gl, {size: 2, data: new Float32Array(positions)});
