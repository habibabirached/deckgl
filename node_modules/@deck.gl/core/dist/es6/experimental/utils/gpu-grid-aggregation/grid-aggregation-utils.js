...
const R_EARTH = 6378000; // Takes data and aggregation params and returns aggregated data.
...
export function pointToDensityGridData({
  ...
  let gridData = {};
  assert(aggregationFlags.dataChanged || aggregationFlags.cellSizeChanged || aggregationFlags.viewportChanged);
...
function parseGridData(data, getPosition, getWeight = null) {
  ...
  for (let p = 0; p < data.length; p++) {
    ...
    const weight = getWeight ? getWeight(data[p]) : [1.0, 0, 0]; // Aggregator expects each weight is an array of size 3
...
function getGPUAggregationParams({
  ...
  const originY = alignToCell(yMin - worldOrigin[1], cellSize[1]) + worldOrigin[1]; // Setup transformation matrix so that every point is in +ve range

  const gridTransformMatrix = new Matrix4().translate([-1 * originX, -1 * originY, 0]); // const cellSize = [gridOffset.xOffset, gridOffset.yOffset];
